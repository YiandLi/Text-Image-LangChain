...
class CLIP(nn.Module):
    def __init__(self, model_name, clip_model=None, clip_processor=None, clip_tokenizer=None):
        ...
        if not clip_model:
            print(f"Re-load(from_pretrained) {model_name} CLIPModel when init captioning model")
            self.model = CLIPModel.from_pretrained(model_name)
        self.model.eval()
        
        if not clip_processor:
            print(f"Re-load(from_pretrained) {model_name} CLIPProcessor when init captioning model")
            self.processor = CLIPProcessor.from_pretrained(model_name)
        
        if not clip_tokenizer:
            print(f"Re-load(from_pretrained) {model_name} CLIPTokenizer when init captioning model")
