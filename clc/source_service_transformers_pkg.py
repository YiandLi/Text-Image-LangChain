...
class SourceService(object):
    def __init__(self, config, clip_model_name=r"openai/clip-vit-base-patch32"):
        ...
        self.img_caption_ratio = 5
        self.init_source_vector()
    
    def get_img_emb(self, img):
        
        with torch.no_grad():
            img_list = Image.open(img.metadata.get('source'))
            ...
            image_embeds = image_embeds[0].detach().cpu().numpy()
            
            if self.img_caption_ratio and img.metadata.get("caption"):
                cap_embeddings = self.get_txt_emb(img.metadata.get("caption"))
                embeddings = image_embeds + cap_embeddings * self.img_caption_ratio
                embeddings = embeddings / np.linalg.norm(embeddings, axis=0, keepdims=True)  # ndarray normalize
            
            return embeddings.tolist()  # 返回一个样本的 hidden_size - d vector
    ...
    def get_txt_emb(self, txt):
        ...
        with torch.no_grad():
            text_inputs = self.tokenizer([txt], padding=True, return_tensors="pt",
            ...
            return text_embeds[0].detach().cpu().numpy()  # 返回一个样本的 hidden_size - d vector
